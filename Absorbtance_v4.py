...
def get_arrayproperties (current):
    # Important import filter spectra
    #with open("tao's project/Code and data(TAO)/filter/var/c=" + str(C) + '(forward)/' + str(current + 1) + '.csv', 'r', encoding='utf-8-sig') as f:
    with open('filter/svd/setA/c='+str(C)+'(forward)/'+str(current+1)+'.csv', 'r', encoding='utf-8-sig') as f:
    ...
    starting_frequency=450#500
    ending_frequency=3970#800
...
with open('svd/filter_rec_a.csv', 'r', encoding='utf-8-sig') as f:
    filter_rec_array = np.genfromtxt(f, dtype=float, delimiter=',', skip_header=False)
filter_rec = list(filter_rec_array)
filter_set = filter_rec[0:4]
precision_list =[]

for current in range(4,len(filter_rec)):
    filter_set_tem=copy.deepcopy(filter_set)
    filter_set_tem.append(filter_rec[current])
    filter = []
    x=Start
    while x <=End:
        trans = []
        for b in filter_set_tem:
            trans.append(A*math.exp((-pow((10000000/x-10000000/b),2))/(2*pow(C,2))))
            #trans.append(A * math.exp((-pow((x -  b), 2)) / (2 * pow(C, 2))))
        filter.append(trans)
        x += 4#0.3
    #with open("tao's project/Code and data(TAO)/filter/var/c=" + str(C) + '(forward)/' + str(current + 1) + '.csv', 'w', newline='') as file:
    with open('filter/svd/setA/c='+str(C)+'(forward)/'+str(current+1)+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(filter)
    (filter_properties, axis_cm)=get_arrayproperties(current)
    with open('spetrum/transmittance_A.csv', 'r', encoding='utf-8-sig') as f:
    #with open("tao's project/Code and data(TAO)/data/fluorophore.csv", 'r', encoding='utf-8-sig') as f:
    ...
    print('number of filters is '+str(len(filter_set_tem)))

    confusion_result=confusion_matrix( flo_id_GT, flo_id_pred)
    #print(confusion_result) 临时
    ...
    #     print(f"{label+1:5d} {precision(label, confusion_result):9.3f} ")
    #临时

    print("precision total:", precision_macro_average(confusion_result))

    if len(precision_list)!=0:
        if precision_list[len(precision_list)-1] > 0.9:
            if precision_macro_average(confusion_result) - precision_list[len(precision_list) - 1] < 0.005:
                print("dropped")
            else:
                print("added")
                precision_list.append(precision_macro_average(confusion_result))
                filter_set = copy.deepcopy(filter_set_tem)
                print(filter_set)
        elif precision_list[len(precision_list)-1] > 0.8:
            improvement = precision_macro_average(confusion_result) - precision_list[len(precision_list) - 1]
            if improvement < 0.01:
                print("dropped")
            else:
                print("The performance of the model increases by "+str(round(improvement,5)*100)+"%")
                print("filter added")
                precision_list.append(precision_macro_average(confusion_result))
                filter_set = copy.deepcopy(filter_set_tem)
                print("The list of centers of filters")
                print(filter_set)
        else:
            improvement = precision_macro_average(confusion_result) - precision_list[len(precision_list) - 1]
            if improvement<0.02:
